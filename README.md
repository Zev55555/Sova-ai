...
它不是普通的 Chat with CSV 工具，也不是让大模型自由生成 SQL 并直接执行的 SQL Agent。

SOVA AI 的核心设计是：

```text
LLM 负责：业务理解、问题澄清、字段语义识别、证据链表达、报告草稿生成
Metric Spec + DuckDB 负责：安全、可控、可复查的数据计算
```
