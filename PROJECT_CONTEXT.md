...
SOVA AI｜指标异动分析工作台 是一个面向中文用户的 AI 指标异动分析 Agent。
...
SOVA AI 面向通用业务指标异动场景，不应被设计成只分析优惠券核销率。
...
UI-3 聚焦前端交互展示，在不修改 backend、API、上传接口、Metric Spec Builder / Executor、证据链、报告或数据计算逻辑的前提下，为上传数据、生成分析计划、执行指标计算、生成证据链和生成报告草稿增加过程反馈。

新增 `frontend/lib/task-feedback.ts`，定义前端任务状态 `idle / running / success / error`、任务 ID、任务标题和 3-5 个子步骤。`frontend/components/metricflow-workspace.tsx` 在主区域当前步骤卡片内显示任务进度卡片，并在右侧分析驾驶舱显示当前正在执行的任务和当前子步骤。失败时保留当前步骤和旧结果，方便用户重试；成功后沿用既有 workflow 自动进入下一步。

## 27. UI-4 结果区信息层级重排

UI-4 聚焦前端结果展示层级，不改 backend、API、上传接口、Metric Spec Builder / Executor、证据链、报告或数据计算逻辑。Step 8 将指标计算结果重排为主结论区、解释区、辅助指标对比和折叠维度拆解，突出核心指标变化、分子 / 分母变化、Top movers 前 5 个分组和辅助指标同步变化。

旧基础分析继续保留为底部默认折叠的辅助探索区。Step 9 证据链强化为可追踪发现列表，并在基于指标计算结果生成时显示来源提示。Step 10 报告草稿强化为正式输出预览，增加报告状态和章节层级标签。

## 28. UI-5 视觉收尾、空状态与首次使用体验

UI-5 聚焦前端视觉一致性和首次使用体验，不改 backend、API、上传接口、Metric Spec Builder / Executor、证据链、报告或数据计算逻辑。顶部入口文案调整为正式产品说明，API 设置入口保留但视觉上弱化。

Step 1 保持业务问题输入框优先；四个示例问题（SaaS 新用户 7 日激活率下降、客服 SLA 超时率上升、物流配送延迟率上升、Valorant 排位胜率下降）收纳在默认折叠的“查看示例问题”辅助入口中。点击示例只会填入业务问题，不会自动提交。按钮、卡片、空状态、小屏下右侧驾驶舱、步骤导航、字段样例表和报告正文都做了轻量统一与降噪处理。
